      ...
      <div className="flex flex-col items-center w-full max-w-xl text-center flex-1 justify-center py-12 relative z-10">
        <div className="flex flex-col items-center justify-center mb-12">
          <div className="relative">
            <div className="w-32 h-32 bg-primary/10 rounded-full flex items-center justify-center border-2 border-primary/20 shadow-[0_0_50px_rgba(16,185,129,0.1)]"><Heart size={64} fill="currentColor" className="text-primary animate-pulse-heart drop-shadow-[0_0_20px_rgba(16,185,129,0.5)]" /></div>
          </div>
          <h1 className="mt-8 text-5xl md:text-6xl font-black tracking-tighter uppercase leading-none text-white text-center">STAY<span className="text-primary">ON</span>BEAT</h1>
        ...
        <div className="flex flex-col gap-4 w-full max-w-sm">
          <button onClick={() => router.push('/auth?mode=signup')} className="pill-button w-full bg-primary text-black text-xl font-black neon-glow active:scale-95 flex items-center justify-center gap-3 shadow-lg shadow-primary/20 uppercase tracking-[0.1em]">{content[lang].getStarted}</button>
          <button onClick={() => router.push('/auth?mode=signin')} className="pill-button w-full bg-white/5 border-2 border-primary/20 text-primary text-lg font-black hover:bg-white/10 transition-all active:scale-95 uppercase tracking-[0.1em]">{content[lang].signIn}</button>
        </div>
      </div>

      <footer className="w-full text-center mt-12 relative z-10"><p className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em]">{content[lang].footer}</p></footer>
    </main>
  );
}
