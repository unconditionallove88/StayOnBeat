  ...
  return (
    <div className="w-full min-h-[85vh] flex flex-col items-center justify-center font-headline max-w-xl mx-auto px-4 text-center relative">
      ...
      <div className="flex-1 w-full overflow-y-auto max-h-[55vh] custom-scrollbar pr-2 mb-8 space-y-8">
        <div className="space-y-3"><h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#3EB489] text-left px-2">{t.res}</h3><div className="grid grid-cols-1 gap-3">{RESONANCE_GOALS.map((goal) => (<button key={goal.id} onClick={() => toggleGoal(goal.id)} className={cn("p-5 rounded-[1.5rem] border-2 flex items-center gap-6 transition-all active:scale-[0.98] text-left", selected.includes(goal.id) ? 'bg-[#3EB489]/10 border-[#3EB489] shadow-[0_0_20px_rgba(62,180,137,0.2)]' : 'bg-[#0a0a0a] border-white/10 hover:border-white/30')}><div className={cn("p-3 rounded-xl", selected.includes(goal.id) ? 'bg-[#3EB489] text-black' : 'bg-white/5 text-white/40')}><goal.icon className="w-5 h-5" /></div><span className={cn("font-black text-base uppercase tracking-tight", selected.includes(goal.id) ? 'text-white' : 'text-white/60')}>{lang === 'EN' ? goal.label : lang === 'DE' ? goal.de : lang === 'PT' ? goal.pt : goal.ru}</span></button>))}</div></div>
        <div className="space-y-3"><h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 text-left px-2">{t.pos}</h3><div className="grid grid-cols-1 gap-3">{POSITIVE_GOALS.map((goal) => (<button key={goal.id} onClick={() => toggleGoal(goal.id)} className={cn("p-5 rounded-[1.5rem] border-2 flex items-center gap-6 transition-all active:scale-[0.98] text-left", selected.includes(goal.id) ? 'bg-[#3EB489]/10 border-[#3EB489] neon-glow' : 'bg-[#0a0a0a] border-white/10 hover:border-white/30')}><div className={cn("p-3 rounded-xl", selected.includes(goal.id) ? 'bg-[#3EB489] text-black' : 'bg-white/5 text-white/40')}><goal.icon className="w-5 h-5" /></div><span className={cn("font-black text-base uppercase tracking-tight", selected.includes(goal.id) ? 'text-white' : 'text-white/60')}>{lang === 'EN' ? goal.label : lang === 'DE' ? goal.de : lang === 'PT' ? goal.pt : goal.ru}</span></button>))}</div></div>
      ...
      <button onClick={() => onComplete(selected)} disabled={selected.length === 0} className={cn("pill-button w-full max-w-sm uppercase tracking-[0.2em] font-black text-xl h-[64px] transition-all", selected.length > 0 ? 'bg-[#3EB489] text-black neon-glow active:scale-95' : 'bg-white/10 text-white/10 cursor-not-allowed border-2 border-white/5 opacity-50')}>{t.confirm}</button>
    </div>
  );
}
