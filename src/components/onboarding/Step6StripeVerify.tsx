...
export function Step6StripeVerify({ onComplete, onBack }: Step6StripeVerifyProps) {
  ...
  const handleSimulateStripe = async () => {
    ...
    try {
      ...
      setError("Verification couldn’t complete right now — please try again.");
  ...
  if (isSuccess) {
    return (
      <div className="w-full min-h-[80vh] flex flex-col items-center justify-center text-center px-6 font-headline animate-in fade-in zoom-in-95 duration-1000">
        ...
        <h2 className="text-4xl font-black uppercase tracking-tighter text-white mb-4">Identity Verified 💚</h2>
        <p className="text-white/60 text-lg font-bold max-w-sm leading-tight">Thank you for helping us keep this sanctuary safe. Your account is now calibrated.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-h-[85vh] flex flex-col items-center justify-center font-headline max-xl mx-auto px-4 text-center relative">
      ...
      <div className="mt-12 mb-8 text-center">
        ...
        <h1 className="text-[28px] font-black uppercase mb-2 text-white leading-tight tracking-tighter">A Safe Space for Adults</h1>
        <p className="text-white/40 font-black uppercase tracking-[0.2em] text-[10px] max-w-[280px] mx-auto">
          To honor German safety standards and protect our community, we verify your age via a secure payment method.
        </p>
      </div>

      <div className="w-full space-y-4 mb-10">
        <div className="p-8 bg-[#0a0a0a] rounded-[2rem] border-2 border-white/10 text-left">
          <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em] mb-6">Secure Verification via Stripe</p>
        ...
        <div className="flex items-start gap-4 p-5 bg-blue-50/5 rounded-2xl border border-blue-500/20 text-left">
          ...
          <p className="text-[10px] text-blue-200/60 font-bold leading-relaxed uppercase tracking-widest leading-tight">
            <strong className="text-blue-400">Your peace of mind matters.</strong> No charge will be made. This is a zero-euro authorization. Data is encrypted and never stored by StayOnBeat.
          </p>
        </div>
      ...
      >
        {isVerifying ? <Loader2 className="w-6 h-6 animate-spin mx-auto" /> : "Verify My Identity"}
      </button>
      <p className="text-[8px] font-bold text-white/20 uppercase tracking-[0.4em] mt-4">PCI-DSS Compliant • Encrypted • Secure</p>
    </div>
  );
}