    ...
    <div className="w-full flex flex-col items-center animate-in fade-in zoom-in duration-1000 min-h-[70vh] justify-center font-headline">
      ...
      <div className="text-center mb-16 space-y-4">
        ...
        <p className="text-white/40 font-bold uppercase tracking-[0.3em] text-sm md:text-base">
          PROFILE READY • PROCEED TO FINAL HEART CHECK
        </p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full mb-20">
        {[
          { label: 'Health Profile', icon: Activity, count: (data.medications?.length || 0) + (data.healthConditions?.length || 0) },
          { label: 'Safety Links', icon: Users, count: 1 },
          { label: 'Substance Lab', icon: Shield, count: data.substances?.length || 0 },
          { label: 'Emergency Contact', icon: Phone, count: data.emergencyContact ? 1 : 0 },
        ].map((item) => (
      ...
      >
        Final Heart Check <ArrowRight className="w-8 h-8" />
      </button>
    </div>
  );
}
