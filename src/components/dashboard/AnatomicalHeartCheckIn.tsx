...
/**
 * @fileOverview AnatomicalHeartCheckIn Component (Reverted).
 * Original design with buttons floating inside the anatomical heart SVG.
 * Persists status to Firestore.
 */
...
export function AnatomicalHeartCheckIn() {
  ...
  // Sync internal state with Firestore when profile loads
  useEffect(() => {
  ...
  const statuses = [
    { id: "steady", label: "Steady", color: "#10B981", emoji: "🍃" },
    ...
    { id: "glowing", label: "Glowing", color: "#8B5CF6", emoji: "✨" },
  ];
  ...
  return (
    <div className="flex flex-col items-center p-8 bg-[#0a0a0a] rounded-[3rem] border border-white/10 shadow-2xl font-headline w-full max-w-lg mx-auto">
      ...
      <div className="relative w-64 h-80 flex items-center justify-center">
        {/* The Anatomical Heart SVG Frame */}
        ...
        {/* The Status Buttons (Floating inside the Heart) */}
        <div className="relative z-10 flex flex-col gap-3 items-center">
          {statuses.map((s) => (
            <button
              key={s.id}
              onClick={() => handleSelect(s)}
              className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-500 border-2 ${
                status === s.label 
