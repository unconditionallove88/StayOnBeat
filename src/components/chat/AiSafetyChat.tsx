...
const CONTENT = {
  en: {
    ...
    water: "💧 Water check", banana: "🍌 Banana / Magnesium", placeholder: "Ask a safety question...", analyzing: "Analyzing risk factors...",
  ...
  de: {
    ...
    water: "💧 Wasser-Check", banana: "🍌 Banane / Magnesium", placeholder: "Sicherheits-Frage stellen...", analyzing: "Risiken werden sanft geprüft...",
  ...
  pt: {
    ...
    water: "💧 Água", banana: "🍌 Banana / Magnésio", placeholder: "Faça uma pergunta...", analyzing: "Analisando riscos...",
  ...
  ru: {
    ...
    water: "💧 Вода", banana: "🍌 Банан / Магний", placeholder: "Задай вопрос...", analyzing: "Анализирую риски...",
    ...
    <div className="flex flex-col h-full bg-black font-body overflow-hidden">
      {currentIntake && (
        <div className="bg-blue-600/10 border-b border-blue-500/20 px-8 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3"><Info size={14} className="text-blue-400" /><span className="text-[9px] font-black uppercase tracking-widest text-blue-400">{t.context}: {currentIntake}</span></div>
        ...
        <div className="space-y-8 max-w-2xl mx-auto pb-10">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
              <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center border border-white/10"><Bot className="w-8 h-8 text-white/20" /></div>
              <div className="space-y-2"><p className="text-lg font-bold text-white/80">{t.question}</p><p className="text-sm text-white/40">{t.sub}</p></div>
              <div className="flex flex-wrap gap-2 justify-center pt-4">
                <div className="px-4 py-2 bg-blue-500/10 border border-blue-500/30 rounded-full flex items-center gap-2"><span className="text-[10px] font-black uppercase text-blue-400">{t.water}</span></div>
                <div className="px-4 py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-full flex items-center gap-2"><span className="text-[10px] font-black uppercase text-yellow-400">{t.banana}</span></div>
              </div>
            </div>
          )}
          {messages.map((msg, i) => (
